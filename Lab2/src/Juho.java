public class Juho
{
	public static void main(String[] args)
	{
		...
		/** TASK: It is important to know the number of spice traders per city. 
		 * So you need to print out the number of traders for all cities (content of the array) 
		 * for each month.
		...
		int[] theNumberOfMonths = new int[patients[0].length];
		int sum = 0;
		for(int i=0;i<theNumberOfMonths.length;i++) {
			for(int j=0;j<ROWS;j++) {
				sum += patients[j][i];
			} theNumberOfMonths[i] = sum;
			System.out.printf("%-8s", theNumberOfMonths[i]);
			sum = 0;}
